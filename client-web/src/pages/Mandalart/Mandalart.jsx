...
class Mandalart extends React.Component {
	componentDidMount() {
		...
		this.props.fetchLog(
			...
			this.props.match.params.mandalId
		);
	}
	...
	renderSmallGrid = (miniMandal, key_i) => {
		...
		const className =
			key_i === 4 ? "mini-container center-mandal" : "mini-container";
		return (
	...
	renderBigGrid = () => {
		...
		return (
			<div className="mandal-big-grid">
				{this.props.mandal.miniMandals.map((miniMandal, i) =>
					this.renderSmallGrid(miniMandal, i)
				)}
			</div>
		);
	};
		...
		return (
			<div className="description-container">
				...
				<div className="contents">
					CONTENTS: {this.props.mandal.mandal.contents}
				</div>
			</div>
		);
	};
...
export default connect(mapStateToProps, { fetchMandal, fetchLog, patchLog })(
	Mandalart
);
