...
app.get("/checklog", async (req, res) => {
	try {
		console.log(
			`GET /checklog?mandalId=${req.query.mandalId}&date=${req.query.date}`
		);
		const { data } = await db.get(
			`/checklogs?mandalId=${req.query.mandalId}&date=${req.query.date}`
		);
		res.status("200").json(data).end(); // 200 == success
	} catch (err) {
		console.log(err);
...
app.get("/checklogs", async (req, res) => {
	try {
		console.log(
			`GET /checklogs?userId=${req.query.userId}&month=${req.query.month}`
		);
		const { data } = await db.get(
			`/checklogs?userId=${req.query.userId}&month=${req.query.month}`
		);
		const result = [];
		data.forEach((row) => {
